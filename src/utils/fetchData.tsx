import { collection, getDocs, query, DocumentData, doc, getDoc, DocumentReference } from 'firebase/firestore';
...
export const fetchPlan = async(): Promise<DocumentReference<DocumentData> | null> => {
  try {
    const userId = auth.currentUser?.uid;
    const planCollection = collection(db, `users/${userId}/plan`);
    const planSnapshot = await getDocs(query(planCollection));
    const doc = planSnapshot.docs[0]
